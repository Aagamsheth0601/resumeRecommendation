...
def usersignupcheck():
    if request.method == 'POST':
        ...
        exists = db.session.query(User.sno).filter_by(username=username).first() is not None
        if exists:
...
def companysignupcheck():
    if request.method == 'POST':
        ...
        if password != confirmpassword:
            return json.dumps({'status':'fail','message':'Password and confirm password do not match'})
        # To check if user already exists
        exists = db.session.query(Company.sno).filter_by(username=username).first() is not None
        if exists:
            return json.dumps({'status':'fail','message':'User already exists. Try different loginID or goto login'})
